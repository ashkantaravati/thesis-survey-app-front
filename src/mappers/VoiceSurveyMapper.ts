...
export default class VoiceSurveyMapper implements IDtoFromModelMapper {
  createDtoFromModel(
    model: Array<MultiResponseLikertScaleQuestion>
  ): Array<VoiceSurveyResponseSchema> {
    const _memberMapper = new TeamMemberMapper();
    const q1 = model.find(
      (q) => q.index === 1
    ) as MultiResponseLikertScaleQuestion;
    const q2 = model.find(
      (q) => q.index === 2
    ) as MultiResponseLikertScaleQuestion;
    const q3 = model.find(
    ...
    const result = ratees.map(function (ratee) {
      const r1 = q1.response.find(
        (r) => r.evaluatedParticipant.id == ratee.id
      ) as LikertResponseWithRatee;
      const r2 = q2.response.find(
        (r) => r.evaluatedParticipant.id == ratee.id
      ) as LikertResponseWithRatee;
      const r3 = q3.response.find(
        (r) => r.evaluatedParticipant.id == ratee.id
      ) as LikertResponseWithRatee;
      const r4 = q4.response.find(
        (r) => r.evaluatedParticipant.id == ratee.id
      ) as LikertResponseWithRatee;
      const r5 = q5.response.find(
        (r) => r.evaluatedParticipant.id == ratee.id
      ) as LikertResponseWithRatee;
      const r6 = q6.response.find(
        (r) => r.evaluatedParticipant.id == ratee.id
      ) as LikertResponseWithRatee;
